                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="">
                        <div class="mb-4">
                            ...
                            <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" wire:model="facture_id" id="facture_id" class="form-control">
                                <option>Selectionnez votre facture</option>
                                @foreach($les_factures as $le_facture)
                                    <option value="{!! $le_facture->id !!}"
                                            wire:key="{{$le_facture->id}}"
                                            {{ $le_facture->id == $facture_id ? 'selected' : ''}}>
                                        {!! $le_facture['numero_facture'] !!}
                                    </option>
                                @endforeach
                            </select>
                            @error('facture_id') <span class="text-red-500">{{ $message }}</span>@enderror
                        </div>
                        <div class="mb-4">
